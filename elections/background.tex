...
\begin{algorithm}
    ...
    \Fn{\stv{ballots, seats, candidates}}{
        ...
        \While{length of elected < seats}{
            \While{There are candidates where $vc_i$ > $dq$}{
                let $i$ = candidate with maximum $vc_i$ value\\
                \If{$vc_i$ > $dq$}{
                    add $i$ to elected\\
                    let $t$ = transfer value for $i$\\
                    divide the votes for $i$ amongst its second preferences
                }
            }
        }
    }
